...
class Generator(object):
    ...
    def __init__(self):
        ...
        with open(self.p.train_root_url + 'label_data_0313.json') as f:
            while True:
                ...
                self.train_data_orig.append(jsonString)
        ...
        with open(self.p.train_root_url + 'label_data_16344.json') as f:
            while True:
                line = f.readline()
                if not line:
                    break
                jsonString = json.loads(line)
                self.train_data_synth.append(jsonString)

        random.shuffle(self.train_data_orig)
        random.shuffle(self.train_data_synth)

        print("Split percentage: {}\% from Original dataset and {}\% from Synthetic dataset".format(
            self.p.split_perc * 100, (1 - self.p.split_perc) * 100))
        num_orig = int(len(self.train_data_orig) * self.p.split_perc)
        num_synth = int(len(self.train_data_synth) * (1 - self.p.split_perc))

        if num_orig != 0:
            print("Number of original examples: ", num_orig)
            self.train_data.extend(self.train_data_orig[0:num_orig])

        if num_synth != 0:
            print("Number of synthetic examples: ", num_synth)
            self.train_data.extend(self.train_data_synth[0:num_synth])

        self.size_train = len(self.train_data)
        print("Length of Train: ", self.size_train)
        ...
        print("Length of Test: ", self.size_test)
    ...
    def split(self):
        ...
        for i in range(self.size_train):
            print("{}/{}".format(i, self.size_train))
