...
def Training():
    ...
    if p.model_path == "":
        ...
        lane_agent = agent.Agent()
        lane_agent.load_weights(model_number, model_name)
    ...
    ##############################
    print('Training loop')
    step = model_number * 100
    sampling_list = None
    len_steps = 0

    for epoch in range(p.n_epoch):
        lane_agent.training_mode()
        for inputs, target_lanes, target_h, test_image, data_list, len_cuts in loader.Generate(sampling_list):
            # training
            ...
            print("epoch: {}/{}".format(str(int(len_steps/len_cuts)), p.n_epoch))
            print("step: {}/{}".format(str(step), str(len_steps)))
        ...
        if epoch >= 0 and epoch % 5 == 0:
            ...
            for idx in index:
                print("compute score")
                with open("evaluation/eval_result_" + str(idx) + "_.txt", 'a') as make_file:
                    make_file.write("epoch : " + str(epoch) + " loss : " + str(loss_p.cpu().data))
                    make_file.write(
                        evaluation.LaneEval.bench_one_submit("test_result_" + str(epoch) + "_" + str(idx) + ".json",
                                                             "test_label.json"))
                    make_file.write("\n")
