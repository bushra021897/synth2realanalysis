...
def get_tusimple_list_test(root, label_list):
    '''
...
def get_tusimple_list(root, label_list, synth_data, split_perc=1.0):
    '''
    Get all the files' names from the json annotation
    '''
    label_json_train = []
    label_json_all = []
    label_json_synth = [json.loads(line) for line in open(os.path.join(root, synth_data)).readlines()]

    for l in label_list:
        l = os.path.join(root, l)
        label_json = [json.loads(line) for line in open(l).readlines()]
        label_json_train += label_json

    random.shuffle(label_json_train)
    random.shuffle(label_json_synth)

    print("Split percentage: {}\% from Original dataset and {}\% from Synthetic dataset".format(split_perc * 100,
                                                                                                (1 - split_perc) * 100))
    num_orig = int(len(label_json_train) * split_perc)
    num_synth = int(len(label_json_train) * (1 - split_perc))

    if num_orig != 0:
        print("Number of original examples: ", num_orig)
        label_json_all.extend(label_json_train[0:num_orig])

    if num_synth != 0:
        print("Number of synthetic examples: ", num_synth)
        label_json_all.extend(label_json_synth[0:num_synth])
...
if __name__ == "__main__":
    ...
    # training set
    names, line_txt = get_tusimple_list(train_root,
                                        ['label_data_0601.json', 'label_data_0531.json', 'label_data_0313.json'],
                                        'label_data_16344.json', split_perc=0.6)

    # generate segmentation and training list for training
    generate_segmentation_and_train_list(train_root, line_txt, names)

    # testing set
    names, line_txt = get_tusimple_list_test(test_root, ['test_label.json'])

    # generate testing set for testing
    with open(os.path.join(test_root, 'test.txt'), 'w') as fp:
        for name in names:
            fp.write(name + '\n')
